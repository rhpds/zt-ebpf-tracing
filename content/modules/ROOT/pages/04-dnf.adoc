...
The first thing that `+dnf+` does is updates repository metadata. This
...
subscription.rhsm.redhat.com, and cdn.redhat.com. `+gethostlatency+`
...
Look at your _tcplife_ tab when `+dnf+` has finished downloading
packages. You will see output similar to the sample below:

The output shows you that `+dnf+` has established a connection on port
...
seconds. This correlates to the amount of time `+dnf+` spent downloading
packages.
